...
def continuous_CSM(map, pose, m_i, sigma, l, z, i, k):
    ...
    for idx in indexes[:1]:
        ...
        if d1 < l:
            map['alpha'][i] = map['alpha'][i] + kernel(d1, sigma, l)
        else:
            minDist = float("inf")
            step_size = l * (2/3)
            ...
            for xl in np.arange(0, dist, step_size):
                ...
                if d2 < minDist:
                    minDist = d2
            if minDist < 1:
                map['beta'][i] = map['beta'][i] + kernel(minDist, sigma, l)
...
class ogm_continuous_CSM:
    ...
    def __init__(self, grid_size = 0.135):
        ...
        # -----------------------------------------------
        self.prior = 0.000001            # prior for setting up mean and variance
    ...
    def construct_map(self, pose, scan):
        ...
        self.map['alpha'] = np.array([self.prior_alpha] * self.map['size']).reshape(-1, 1)      # size should be (number of data) x (1)
        self.map['beta'] = np.array([self.prior_alpha] * self.map['size']).reshape(-1, 1)       # size should be (number of data) x (1)
