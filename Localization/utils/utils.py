...
def func(x):
   ...
   ###############################################################################
    # TODO: Implement the func function for extra credits                         #
...
def lieToCartesian(mean, cov):
    ...
    kappa = 2
    mu_cart, Sigma_cart = unscented_propagate(mean, cov, kappa)
...
def mahalanobis(state, ground_truth, filter_name, Lie2Cart):
    ...
    if filter_name == 'EKF' or filter_name == 'UKF' or filter_name == 'PF':
        mu = state.getState()
        cov = state.getCovariance()

    if filter_name == 'InEKF':
        if Lie2Cart is False:
            mu = state.getState()
            cov = state.getCovariance()
        else:
    ...
    error = mu - ground_truth
    error[2] = wrap2Pi(error[2])
    dist_ma = np.sqrt(error.T @ np.linalg.inv(cov) @ error)
    sigma_3 = 3 * np.sqrt(np.diag(cov))

    # results = np.hstack((dist_ma, error, cov))
    results = np.hstack((dist_ma, error, sigma_3))
    ...
    # results[5] = (cov[1][1])
    # results[6] = (cov[2][2])
    # results[4] = 3 * np.sqrt(cov[0][0])
    # results[5] = 3 * np.sqrt(cov[1][1])
    # results[6] = 3 * np.sqrt(cov[2][2])
